...
The same error now refuses inherited model fields and indexed models rather than silently emitting only own fields or an empty message. It also refuses explicit TypeSpec defaults, compiler validation constraints (length, pattern, format, numeric and collection bounds), `@encode`, restricted lifecycle visibility, `@discriminator` and `@discriminated` on reached declarations. Constraints and encodings on custom scalar base chains are included. The diagnostic points at the unsupported declaration. Declare a separate binary wire type and enforce application rules separately; no partial artifact or JSON fallback is written.

Unconstrained mapped scalars, documentation, all lifecycle phases and `@encodedName("application/json", ...)` remain accepted. JSON-only encoded names do not rename binary fields. The proto3 field name remains the TypeSpec spelling and the tag comes from `@Protobuf.field`.

## Binary reader/writer evidence

The baseline tests compile old and new TypeSpec sources independently with the explicit root `contract.Event`. They parse actual emitted texts independently with `protobufjs` and `keepCase: true`, verify writer inputs, encode with that writer, decode with the selected reader, and assert both values and own-field presence. The official emitter descriptor-parity oracle remains separate.

| Mutation                              | What successful decoding does **not** imply                                                                                                                                                                                                                                                                                                           |
| ------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Optional/required addition or removal | Unknown tags are lost; a missing field may decode without an own property. TypeSpec requiredness is an application check, not a proto3 required wire label.                                                                                                                                                                                           |
| Explicit source default               | Generation is refused, including an explicitly written scalar-zero default. No migration adapter or arbitrary TypeSpec default is installed. Implicit proto3 defaults are different.                                                                                                                                                                  |
| Optionality                           | Explicit optional scalar-zero values retain presence with an optional writer and reader. The installed `protobufjs` drops scalar-zero own-field presence for implicit-presence fields, including on read. Missing values and `null` input do not establish nullable binary semantics; repeated and map fields do not have singular optional presence. |
| Rename or tag change                  | A stable tag/type can preserve a value under a different source field name. Changing the tag may instead decode with complete value loss. Retired tags/names should be reserved.                                                                                                                                                                      |
| Enum change                           | An old reader can retain an unknown numeric enum value while a closed-enum consumer check rejects it. This is not symbolic or ProtoJSON compatibility.                                                                                                                                                                                                |
| Scalar type change                    | `int32`/`sint32` can decode each other's data with different values; a wide `int64` can truncate in an `int32` reader. Decoder success is not value preservation.                                                                                                                                                                                     |

Each supported mutation runs old→old, old→new, new→old and new→new. Rejections and accepted-with-loss/default outcomes are asserted, not skipped or labelled universally safe. Unknown fields dropped by decoding cannot be recovered by re-encoding. Writer verification is not a source-contract validator and does not prove all numeric ranges or requiredness; wide integers need exact library representations, not precision-losing JavaScript numbers.

The official AsyncAPI parser validates structure and schema syntax, not instances. Separate `@headers` remain native JSON schemas; lifted `@header` fields on generated payloads are refused. An authored proto2 control has different wire-required/default semantics. An explicit root can be decoded even when mutually recursive messages prevent the official AsyncAPI parser from inferring one.

This is finite baseline evidence for these codecs, values and consumer checks, not a universal compatibility proof, version-decorator projection test, registry policy, HTTP/ProtoJSON contract or broker delivery guarantee.
