...
This workspace targets `0.4.x` for the next coordinated release with TypeSpec compiler `^1.16.0`. Until that release is published, use the repository's workspace package with `pnpm install --frozen-lockfile` and `pnpm build`, not the registry command above. `tsp-avro` is not yet at 1.0, so its decorators can still change. The supported range follows its releases.
